...
export default async function latex(texDoc: string, opts: LaTeXOpts) {
  ...
  try {
    ...
    resetEngines()
    throw new Error(
      'Unable to compile the resume PDF. Please try again or choose another template.'
    )
  }
}
...
function resetEngines() {
  for (const engine of [pdftex, xetex, dvipdfmx]) {
    try {
      engine.closeWorker()
    } catch {
      // A failed compile can leave a worker partly released.
    }
  }
  engineLoaded = false
}
